...
local OtherItemsTarget = nil
local SerachActive = false
...
RegisterNetEvent("gui:getOtherItems")
AddEventHandler("gui:getOtherItems", function(THEITEMS)
    OTHERITEMS = THEITEMS
end)
RegisterCommand('szafka', function(source, args)
    TriggerServerEvent("player:getlLocker")
    ActionType = "szafka"
	SerachActive = true
	 Wait(500)
     openInventory()
...
RegisterNetEvent("redmrp_invenotry:openlocker")
AddEventHandler("redmrp_invenotry:openlocker", function()
    TriggerServerEvent("player:getlLocker")
    ActionType = "szafka"
	SerachActive = true
	 Wait(500)
...
RegisterNetEvent("redmrp_invenotry:search")
AddEventHandler("redmrp_invenotry:search", function(target)
    if target ~= 0 then
        Citizen.CreateThread(function()
            ActionType ="przeszukanie"
            local _target  = target
            OtherItemsTarget = _target
            SerachActive = true
            TriggerServerEvent("player:getItems")
            Wait(500)
            TriggerServerEvent("player:getItems", _target)
            Wait(500)
            openInventory()
            loadOtherInventory()
        end)
        ...
        for k,v in pairs(Pickups) do
            ...
            if distance <= 0.7 and not v.inRange  then
                ...
                if PromptHasHoldModeCompleted(PickPrompt) then
                    ...
                    local DisplayName2 =  v.name
                    if v.hash == 1 then
                        if Config.Labels[v.name] ~= nil then
                            DisplayName2 = Config.Labels[v.name]
                        end
                    else
                        DisplayName2 = GetLabelTextByHash(v.hash)
                    end

                    TriggerEvent("pNotify:SendNotification", {
                        text = "<img src='nui://redemrp_inventory/html/img/items/"..v.name..".png' height='40' width='40' style='float:left; margin-bottom:10px; margin-left:20px;' />Pickup: ".. DisplayName2.."<br>+"..tonumber(v.amount),
                        type = "success",
...
RegisterNetEvent('player:removeWeapon')
AddEventHandler('player:removeWeapon', function(ammo , hash)
    Citizen.Wait(1000)
    RemoveWeaponFromPed(PlayerPedId(), tonumber(hash) , false)
end)
...
RegisterNUICallback('HideInventoryAndChange', function()
    isInInventory = false
    SendNUIMessage({
        action = "hide"
    })
...
RegisterNUICallback('Load', function()
    loadPlayerInventory()
    isInInventory = true
    SendNUIMessage({
        action = "display"
...
RegisterNUICallback('TakeItem', function(data, cb)
    print(data.type)
	 local test = 1
    if ActionType == "przeszukanie" then
        if data.type == "item_standard" then
            if tonumber(data.number) > 0 then               
                TriggerServerEvent("item:take", data.item, tonumber(data.number) , test , OtherItemsTarget , data.search)
            end
        else
            Citizen.CreateThread(function()
                TriggerServerEvent("item:take", data.item, GetAmmoInPedWeapon(PlayerPedId(), tonumber(data.hash)), tonumber(data.hash), data.search)
                RemoveWeaponFromPed(PlayerPedId(), tonumber(data.hash) , false)
            end)
        end
    elseif ActionType == "szafka" then
	 if data.type == "item_standard" then
	  OTHERITEMS[data.item] = OTHERITEMS[data.item] - tonumber(data.number)
        TriggerServerEvent("player:SavelLocker" ,OTHERITEMS, data.item, tonumber(data.number), "add", test)
		else
		local ammo = OTHERITEMS[data.item]
		  OTHERITEMS[data.item] = nil
		  TriggerServerEvent("player:SavelLocker" ,OTHERITEMS, data.item,  ammo[1],  "add"  ,ammo[2])
		end
			loadOtherInventory()
    end
end)
...
RegisterNUICallback('DropItem', function(data, cb)
    ...
    if data.type == "item_standard" then
        if tonumber(data.number) > 0 then
            local test = 1
            local DisplayName = data.item
            if Config.Labels[data.item] ~= nil then
                DisplayName = Config.Labels[data.item]
            end
            TriggerServerEvent("item:drop", data.item, tonumber(data.number) , test)
            TriggerEvent("pNotify:SendNotification", {
                text = "<img src='nui://redemrp_inventory/html/img/items/"..data.item..".png' height='40' width='40' style='float:left; margin-bottom:10px; margin-left:20px;' />Droped: ".. DisplayName.."<br>-"..tonumber(data.number),
                type = "success",
                timeout = math.random(2000, 3000),
                layout = "centerRight",
                queue = "right"
        ...
        Citizen.CreateThread(function()
            ...
            RemoveWeaponFromPed(PlayerPedId(), tonumber(data.hash) , false)
            local DisplayName2 = GetLabelTextByHash(tonumber(data.hash))
            local ammo = GetAmmoInPedWeapon(PlayerPedId(), tonumber(data.hash))
            TriggerEvent("pNotify:SendNotification", {
                text = "<img src='nui://redemrp_inventory/html/img/items/"..data.item..".png' height='40' width='40' style='float:left; margin-bottom:10px; margin-left:20px;' />Droped: "..DisplayName2.."<br>-"..tonumber(ammo),
                type = "success",
                timeout = math.random(2000, 3000),
                layout = "centerRight",
                queue = "right"
...
RegisterNUICallback('GiveItem', function(data, cb)
    ...
    if ActionType == "przeszukanie" then
        data.player = OtherItemsTarget
    end
    if ActionType ~= "szafka" then
        for i=1, #players, 1 do
	...
	if data.type == "item_standard" then
		if  OTHERITEMS[data.item] == nil then
			OTHERITEMS[data.item] = tonumber(data.number)
		else
		
			OTHERITEMS[data.item] = OTHERITEMS[data.item] + tonumber(data.number)
		end
   TriggerServerEvent("player:SavelLocker" ,OTHERITEMS, data.item, tonumber(data.number) ,"delete", 1)
  

	else
	 OTHERITEMS[data.item]= {tonumber(GetAmmoInPedWeapon(PlayerPedId(),tonumber(data.hash)))  , tonumber(data.hash)}
	 local ammo = GetAmmoInPedWeapon(PlayerPedId(), tonumber(data.hash))
	 print(ammo)
	TriggerServerEvent("player:SavelLocker" ,OTHERITEMS, data.item, ammo ,"delete", tonumber(data.hash))
	Wait(500)
	RemoveWeaponFromPed(PlayerPedId(), tonumber(data.hash) , true)
	 end
	 loadOtherInventory()
	 end
end)
...
local time = math.random(300000,400000)
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(time)
        if gotItems == true and weaponloaded == true then
            print("save")
            for k, v in pairs(ITEMS) do
                if tonumber(v) == nil then
                    print (k)
            ...
            time = math.random(300000,400000)
        end
    end
end)
